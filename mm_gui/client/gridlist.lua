...
function createGridlist(x, y, w, h, scrollTextures)
	if x and y and w and h then 
		...
		local el = createElement("gridlist", "gridlist"..tostring(listID)) 
...
function destroyGridlist(list)
	if isElement(list) and getElementType(list) == "gridlist" then 
...
function addGridlistItem(list, column, item)
	if isElement(list) and getElementType(list) == "gridlist" and item then
		local data = getElementData(list, "data")
...
function setGridlistFont(list, font, fontSize)
	if isElement(list) and getElementType(list) == "gridlist" and font and fontSize then
		local data = getElementData(list, "data")
		data.font = font 
...
function setGridlistSelectionMode(list, type)
	if isElement(list) and getElementType(list) == "gridlist" and type then 
		local data = getElementData(list, "data")
...
function moveGridlistScroll(list, direction, value)
	if isElement(list) and getElementType(list) == "gridlist" and direction and value then
		local data = getElementData(list, "data")
...
--[[
local list = createGridlist(400, 00, 500, 400, {background=dxCreateTexture(":repair/images/scrollbar.png"), grip=dxCreateTexture(":repair/images/scrollbar_point.png")})
