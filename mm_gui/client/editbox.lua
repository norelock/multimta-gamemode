...
function createEditbox(text, x, y, w, h, font, fontHeight)
	if text and x and y and w and h and font and fontHeight then 
		id = id + 1
		editboxes[id] = {text=text, x=x, y=y, w=w, h=h, font=font, fontHeight=fontHeight, carretPos=1, helperText=false, maxLength=35, masked=false, ascii=true, number=false, barProgress=0, alpha=255}
		return id
	end 
...
function setEditboxText(editboxID, text)
	local edit = editboxes[editboxID]
	if edit then
		editboxes[editboxID].text = text
		return true
	end
...
function setEditboxPosition(editboxID, x, y)
	if editboxes[editboxID] and type(x) == "number" and type(y) == "number" then
...
function changeEditbox(key, press)
